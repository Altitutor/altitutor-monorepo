...
- **Portal access resolver** — A caller-authenticated, app-scoped database read performed by the initial protected Server Component boundary. It returns the minimum routing and hydration facts for one portal in one round trip. It is a user-experience gate; API authorization, RPC checks, and RLS remain authoritative for protected data and mutations.
