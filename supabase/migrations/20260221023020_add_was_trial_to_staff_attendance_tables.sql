-- Migration: Add was_trial tracking to staff attendance tables
-- Description:
--  - Add was_trial BOOLEAN column to sessions_staff table
--  - Add was_trial BOOLEAN column to tutor_logs_staff_attendance table
--  - Create triggers to auto-populate was_trial when records are created
--  - Update create_tutor_log function to set was_trial when inserting staff attendance
--  - Backfill existing data using activity_events or current staff status
-- Purpose: Track whether a staff member was TRIAL at the time of planned/actual attendance

-- ========================
-- ADD was_trial TO sessions_staff
-- ========================
ALTER TABLE public.sessions_staff
  ADD COLUMN IF NOT EXISTS was_trial BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX IF NOT EXISTS idx_sessions_staff_was_trial ON public.sessions_staff(was_trial);

COMMENT ON COLUMN public.sessions_staff.was_trial IS 
  'Indicates whether the staff member was TRIAL status at the time they were added to this session (planned attendance)';

-- ========================
-- ADD was_trial TO tutor_logs_staff_attendance
-- ========================
ALTER TABLE public.tutor_logs_staff_attendance
  ADD COLUMN IF NOT EXISTS was_trial BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX IF NOT EXISTS idx_tutor_logs_staff_attendance_was_trial ON public.tutor_logs_staff_attendance(was_trial);

COMMENT ON COLUMN public.tutor_logs_staff_attendance.was_trial IS 
  'Indicates whether the staff member was TRIAL status at the time attendance was logged (actual attendance)';

-- ========================
-- CREATE TRIGGER FUNCTION FOR sessions_staff
-- ========================
CREATE OR REPLACE FUNCTION public.set_sessions_staff_was_trial()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  -- Set was_trial based on staff status
  IF v_staff_status = 'TRIAL' THEN
    NEW.was_trial := TRUE;
  ELSE
    NEW.was_trial := FALSE;
  END IF;
...
DROP TRIGGER IF EXISTS trigger_sessions_staff_was_trial ON public.sessions_staff;
CREATE TRIGGER trigger_sessions_staff_was_trial
BEFORE INSERT OR UPDATE ON public.sessions_staff
FOR EACH ROW 
EXECUTE FUNCTION public.set_sessions_staff_was_trial();

-- ========================
-- CREATE TRIGGER FUNCTION FOR tutor_logs_staff_attendance
-- ========================
CREATE OR REPLACE FUNCTION public.set_tutor_logs_staff_attendance_was_trial()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trigger_tutor_logs_staff_attendance_was_trial ON public.tutor_logs_staff_attendance;
CREATE TRIGGER trigger_tutor_logs_staff_attendance_was_trial
BEFORE INSERT OR UPDATE ON public.tutor_logs_staff_attendance
FOR EACH ROW 
EXECUTE FUNCTION public.set_tutor_logs_staff_attendance_was_trial();
...
BEGIN
  ...
  -- 2. Create staff attendance records with was_trial
  ...
  LOOP
    ...
    -- Get staff status to determine was_trial
    SELECT status INTO v_staff_status
    FROM staff
    WHERE id = v_staff_id;
    
    -- Set was_trial based on current staff status
...
-- ========================
-- Backfill sessions_staff.was_trial
-- Strategy: Use activity_events to find when staff status changed from TRIAL to ACTIVE
-- For records where session was before status change, mark as TRIAL
-- Otherwise, use current staff status

UPDATE public.sessions_staff ss
SET was_trial = CASE
  -- If staff is currently TRIAL, they were TRIAL
  WHEN EXISTS (
    SELECT 1 FROM public.staff s 
    WHERE s.id = ss.staff_id AND s.status = 'TRIAL'
  ) THEN TRUE
  -- If staff is currently ACTIVE, check if they were TRIAL during the session
  WHEN EXISTS (
    SELECT 1 FROM public.staff s
...
-- Backfill tutor_logs_staff_attendance.was_trial
-- Strategy: Use activity_events to find when staff status changed from TRIAL to ACTIVE
-- For records where tutor log was created before status change, mark as TRIAL
-- Otherwise, use current staff status

UPDATE public.tutor_logs_staff_attendance tlsa
SET was_trial = CASE
  -- If staff is currently TRIAL, they were TRIAL
  WHEN EXISTS (
    SELECT 1 FROM public.staff s 
    WHERE s.id = tlsa.staff_id AND s.status = 'TRIAL'
  ) THEN TRUE
  -- If staff is currently ACTIVE, check if they were TRIAL when tutor log was created
  WHEN EXISTS (
    SELECT 1 FROM public.staff s
    JOIN public.tutor_logs tl ON tl.id = tlsa.tutor_log_id
    WHERE s.id = tlsa.staff_id 
    AND s.status = 'ACTIVE'
    AND EXISTS (
      SELECT 1 FROM public.activity_events ae
      WHERE ae.entity_type = 'staff'
...
-- COMMENTS
-- ========================
COMMENT ON FUNCTION public.set_sessions_staff_was_trial IS 
  'Trigger function to automatically set was_trial based on staff status when sessions_staff record is created';

COMMENT ON FUNCTION public.set_tutor_logs_staff_attendance_was_trial IS 
  'Trigger function to automatically set was_trial based on staff status when tutor_logs_staff_attendance record is created';
