-- Migration: search_students_admin — online subscription + in-person class flags and filters
-- Description: Adds has_online_subscription (any student_subscriptions row) / has_in_person_class (current class enrollment); optional p_subscription_filter and p_in_person_filter ('has' | 'none').
--
-- Adding parameters creates a new overload unless the old signature is dropped; drop the
-- pre-migration 9-argument version so COMMENT and RPC resolve to a single function.
...
COMMENT ON FUNCTION public.search_students_admin(
  ...
  text,
  text
) IS 'Admin search for students. Returns classes with names; has_online_subscription (any subscription row) and has_in_person_class (current enrollment); optional p_subscription_filter / p_in_person_filter.';
