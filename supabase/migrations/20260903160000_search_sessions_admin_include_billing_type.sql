-- Keep the search_sessions_admin session payload aligned with Tables<'sessions'>.
-- billing_type is a generated sessions column, so returning it exposes the
-- authoritative billability classification without duplicating billing rules.
-- CREATE OR REPLACE preserves the function privileges hardened in 20260902140000.
...
BEGIN
  ...
  WITH filtered_sessions AS (
    SELECT s.id, s.type, s.billing_type, s.class_id, s.subject_id, s.admin_shift_id, s.start_at, s.end_at, s.status, s.created_at, s.updated_at
    FROM public.sessions s
  ...
  SELECT jsonb_agg(jsonb_build_object(
      'id', id, 'type', type, 'billing_type', billing_type, 'class_id', class_id, 'subject_id', subject_id,
  ...
  IF p_include_relationships THEN
    ...
    session_students_with_invoice AS (
      SELECT ss.session_id, ss.id as sessions_students_id, st.id as student_id, st.first_name, st.last_name, st.status,
        ...
        ss.credited_at as absence_credited_at,
        ...
        CASE WHEN iss.invoice_status IS NOT NULL THEN jsonb_build_object(
          ...
          'credited_at', iss.invoice_credited_at,          'invoice_id', iss.invoice_id
    ...
    session_students_aggregated AS (
      SELECT session_id, jsonb_agg(jsonb_build_object(
        ...
        'invoice_status_payload', invoice_status_payload, 'is_extra', is_extra,        'absence_credited_at', absence_credited_at
      )) as students
    ...
    IF v_tutor_log_ids IS NOT NULL AND array_length(v_tutor_log_ids, 1) > 0 THEN
      ...
      unplanned_students_aggregated AS (
        SELECT session_id, jsonb_agg(jsonb_build_object(
          ...
          'invoice_status_payload', NULL, 'is_extra', is_extra,        'absence_credited_at', NULL
        )) as students
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS
  'Search sessions. Session payload includes generated billing_type. Swapped staff pill uses replacement row staff. invoice_status_payload includes invoice_id. Students include absence_credited_at (sessions_students.credited_at).';
