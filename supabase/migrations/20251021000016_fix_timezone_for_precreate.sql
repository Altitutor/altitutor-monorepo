-- Migration: Fix timezone handling for precreate_sessions (use Australia/Adelaide)

-- Ensure timezone is set explicitly when constructing timestamptz
  ...
  LOOP
    ...
    WHILE d <= end_date LOOP
      IF EXTRACT(DOW FROM d) = c.day_of_week THEN
        -- Interpret class times as local times on date d in Australia/Adelaide
