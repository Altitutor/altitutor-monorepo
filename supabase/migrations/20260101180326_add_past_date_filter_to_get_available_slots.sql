-- Migration: Add past date filtering and minimum advance booking days to get_available_slots
-- Description: 
--   - Filter out slots in the past
--   - Enforce minimum advance booking days requirement
--   - Adjust start_date to respect minimum advance booking days
...
BEGIN
  ...
  INTO v_slot_duration, v_booking_buffer, v_min_advance_days;
...
COMMENT ON FUNCTION public.get_available_slots IS 'Calculate available booking slots based on opening hours, staff availability, blockouts, existing sessions, and reservations. Filters out past dates and enforces minimum advance booking days requirement.';
