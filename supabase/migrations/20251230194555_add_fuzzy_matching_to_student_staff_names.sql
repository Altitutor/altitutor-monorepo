-- Migration: Add fuzzy matching to student and staff name searches
-- Description:
--   - Update search_students_admin to add fuzzy matching for student names (matching class pattern)
--   - Update search_staff_admin to add fuzzy matching for staff names (matching class pattern)
--   - Ensure search_sessions_admin has exact + fuzzy matching for session shortname (class shortname)
...
BEGIN
  ...
  IF v_search_lower IS NOT NULL THEN
    ...
    FROM (
      ...
      UNION
      -- Search by class names (exact + fuzzy matching - already had this)
...
-- ========================
-- Note: Sessions search already has exact + fuzzy matching for class shortname/longname.
-- This update ensures the pattern is consistent and adds a comment for clarity.
...
COMMENT ON FUNCTION search_students_admin IS 'Admin search function for students with exact + fuzzy name matching and class name search.';
