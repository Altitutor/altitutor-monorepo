-- Add practice-pool membership to the tutor question catalog's server-side filters.
-- The prior function must be dropped because PostgreSQL cannot replace a function signature.
...
REVOKE ALL ON FUNCTION public.tutor_ucat_list_question_catalog(
  TEXT, BOOLEAN, TEXT, TEXT[], UUID[], UUID[], BOOLEAN, UUID[], TEXT[],
  UUID[], BOOLEAN, TEXT[], UUID[], TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT,
  INTEGER, INTEGER, BOOLEAN, TEXT[], INTEGER, INTEGER, BOOLEAN
) FROM PUBLIC, anon;
