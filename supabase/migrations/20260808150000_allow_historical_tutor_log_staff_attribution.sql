-- Allow AdminWeb to create historical tutor logs attributed to staff who are no
-- longer active. The API authorizes the admin actor separately; p_created_by is
-- the staff attribution stored on the log and must only identify a linked staff
-- record so activity triggers can resolve an auth user.
...
COMMENT ON FUNCTION public.create_tutor_log(UUID, UUID, JSONB, JSONB, JSONB, JSONB, JSONB, JSONB) IS
  'Atomically creates a tutor log for an existing linked staff attribution, including inactive historical staff; sets JWT sub from p_created_by for activity events.';
