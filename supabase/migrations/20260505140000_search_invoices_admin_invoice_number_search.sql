-- Server-side substring filter on stripe_invoice_number for admin invoice list.
-- Replaces 8-arg overload with 9-arg version (optional p_invoice_number_search).

DROP FUNCTION IF EXISTS public.search_invoices_admin(
  DATE,
  DATE,
  UUID[],
  TEXT[],
...
CREATE OR REPLACE FUNCTION search_invoices_admin(
  ...
  p_invoice_number_search TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH filtered_invoices AS (
    ...
    WHERE 
      ...
      AND (
        p_invoice_number_search IS NULL
        OR trim(p_invoice_number_search) = ''
        OR position(lower(trim(p_invoice_number_search)) IN lower(coalesce(i.stripe_invoice_number, ''))) > 0
      )
  ),
...
GRANT EXECUTE ON FUNCTION search_invoices_admin(DATE, DATE, UUID[], TEXT[], INTEGER, INTEGER, TEXT, BOOLEAN, TEXT) TO authenticated;

COMMENT ON FUNCTION search_invoices_admin(DATE, DATE, UUID[], TEXT[], INTEGER, INTEGER, TEXT, BOOLEAN, TEXT) IS 'Admin search invoices; excludes soft-deleted. Optional case-insensitive substring match on stripe_invoice_number.';
