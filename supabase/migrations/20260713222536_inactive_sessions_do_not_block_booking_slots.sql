-- Migration: Soft-cancelled (INACTIVE) sessions must not block booking slots
-- Description:
--   - get_available_slots and assign_staff_to_booking previously treated any status
--     other than the unused ''CANCELLED'' value as blocking. Soft cancel sets
--     status = INACTIVE, so availability must only treat ACTIVE sessions as busy.
...
COMMENT ON FUNCTION public.get_available_slots(p_start_date date, p_end_date date, p_session_type public.session_type, p_subject_id uuid, p_duration_minutes integer, p_bypass_date_restrictions boolean) IS 'Calculate available booking slots. Only ACTIVE sessions block availability (INACTIVE/cancelled sessions do not). Supports multiple opening hours ranges per day. ADMIN_SHIFT sessions override day-of-week availability but do not block bookings.';
...
COMMENT ON FUNCTION public.assign_staff_to_booking IS 'Assign staff to booking based on priority. Only ACTIVE sessions are considered for proximity and load balancing.';
