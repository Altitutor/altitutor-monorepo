-- Migration: Fix get_available_slots crash from invalid session time ranges
-- Description:
--   - Add defensive checks to skip sessions/blockouts/reservations with end_at < start_at
--   - Fix corrupted session data where Adelaide times were stored incorrectly (end before start)
-- Root cause: Sessions with end_at before start_at cause tstzrange() to throw
--   "range lower bound must be less than or equal to range upper bound"
-- Related: Production had 46 CLASS sessions with this corruption

-- ========================
-- 1. FIX CORRUPTED SESSION DATA
-- ========================
-- Pattern: start_at had Adelaide time stored as UTC (e.g. 12:30), end_at had correct UTC (e.g. 02:00 = 12:30pm Adelaide)
-- Fix: Swap - use end_at as start (02:00 UTC = 12:30pm Adelaide), add 1.5h for typical class length
UPDATE sessions
SET
  start_at = end_at,
  end_at = end_at + INTERVAL '1 hour 30 minutes'
WHERE end_at < start_at;

-- ========================
-- 2. ADD DEFENSIVE CHECKS TO get_available_slots
-- ========================
-- Skip any sessions/blockouts/reservations with invalid ranges to prevent future crashes
        ...
        FROM (
          ...
          WHERE s.status = 'ACTIVE'
            ...
            AND (
              ...
              -- Defensive: skip invalid ranges (end_at < start_at) to prevent tstzrange crash
              EXISTS (
                ...
                WHERE admin_ss.staff_id = s.id
                  ...
                  AND admin_sess.end_at > admin_sess.start_at
            ...
            -- Defensive: skip invalid ranges to prevent tstzrange crash
            AND NOT EXISTS (
              SELECT 1 FROM sessions sess
              JOIN sessions_staff ss ON ss.session_id = sess.id
              WHERE ss.staff_id = s.id
                AND sess.status != 'CANCELLED'
                AND sess.type != 'ADMIN_SHIFT'
                AND sess.end_at > sess.start_at
