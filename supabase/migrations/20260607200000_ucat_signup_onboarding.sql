-- Migration: UCAT signup onboarding progress on students
-- Adds persisted wizard step, completion timestamp, and test year for the unified
-- /signup/complete flow. Backfills legacy accounts that already chose a plan.
...
COMMENT ON VIEW public.vstudent_ucat_my_access IS
  'UCAT entitlements: online tier, quota exemption, plan onboarding, signup wizard progress.';
