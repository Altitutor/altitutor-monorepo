-- Migration: Fix Uninvoiced Sessions View Timezone and Session Name Format
-- Description:
--  - Fix session_name timezone from UTC to Australia/Adelaide
--  - Update session_name format to "{subject longname} {time h:mm p} {date ddd d MMM} {session type}"
--  - Example: "SACE Year 12 Mathematics 2:30 PM Mon 1 Jan CLASS"
-- Purpose: Display session names in Adelaide local time with proper date format
...
COMMENT ON VIEW public.vadmin_reconciliation_uninvoiced_sessions IS 
  'Admin view: Past sessions without invoice items (need manual invoicing). Includes session name (Adelaide timezone) and attendance fields.';
