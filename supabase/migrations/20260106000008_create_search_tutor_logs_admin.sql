-- Migration: Create search_tutor_logs_admin RPC function
-- Description:
--  - Search and filter tutor logs by session date (not tutor log created date)
--  - Filter by session date using Adelaide timezone
--  - Support searchable staff filter
--  - Return tutor logs with all related data (session, class, subject, staff attendance, student attendance, topics, files)
--  - Support sorting by session date/time

-- ========================
-- CREATE SEARCH_TUTOR_LOGS_ADMIN FUNCTION
-- ========================
...
BEGIN
  ...
  IF v_tutor_log_ids IS NOT NULL AND array_length(v_tutor_log_ids, 1) > 0 THEN
    WITH topic_files_by_log AS (
      ...
      JOIN public.topics_files tf ON tf.id = tltf.topic_file_id
