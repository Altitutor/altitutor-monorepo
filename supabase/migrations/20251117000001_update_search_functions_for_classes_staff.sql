-- Migration: Update search functions to use classes_staff.unassigned_at instead of status
-- Description:
--   - Update all search functions that reference classes_staff.status = 'ACTIVE'
--   - Replace with unassigned_at IS NULL
...
-- ========================
-- UPDATE SEARCH_ALL_ADMIN FUNCTION
-- ========================
...
BEGIN
    ...
    SELECT 
      ...
      jsonb_build_object(
        ...
        'classes', (
          ...
          JOIN classes c ON c.id = cs.class_id
          LEFT JOIN subjects subj ON subj.id = c.subject_id
          WHERE cs.staff_id = st.id AND cs.unassigned_at IS NULL
        )
      ) AS data
    FROM staff st
    ...
    SELECT 
      ...
      jsonb_build_object(
        ...
        'staff', (
          ...
          FROM classes_staff cs
          JOIN staff st ON st.id = cs.staff_id
          WHERE cs.class_id = c.id AND cs.unassigned_at IS NULL
        ),
        'students', (
          SELECT COALESCE(
            jsonb_agg(
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM sorted_results) > (p_limit + p_offset)
