-- Migration: Add p_student_ids and p_staff_ids to search_classes_admin
-- Description: Filter classes by enrolled students and/or assigned staff
-- When p_student_ids is provided: only return classes with at least one of those students enrolled
-- When p_staff_ids is provided: only return classes with at least one of those staff assigned

DROP FUNCTION IF EXISTS search_classes_admin(TEXT, TEXT[], UUID[], BOOLEAN, BOOLEAN, BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN);
...
BEGIN
  ...
  WITH filtered_classes AS (
    ...
    WHERE (v_class_ids IS NULL OR (array_length(v_class_ids, 1) > 0 AND c.id = ANY(v_class_ids)))
      ...
      AND (p_student_ids IS NULL OR array_length(p_student_ids, 1) IS NULL OR EXISTS (
        SELECT 1 FROM classes_students cs
        WHERE cs.class_id = c.id AND cs.unenrolled_at IS NULL AND cs.student_id = ANY(p_student_ids)
      ))
      AND (p_staff_ids IS NULL OR array_length(p_staff_ids, 1) IS NULL OR EXISTS (
        SELECT 1 FROM classes_staff cs
        WHERE cs.class_id = c.id AND cs.unassigned_at IS NULL AND cs.staff_id = ANY(p_staff_ids)
      ))
  ),
...
COMMENT ON FUNCTION search_classes_admin IS 'Admin search function for classes. Search by class short_name and long_name (ILIKE). Supports filtering by subject_ids, student_ids (enrolled), and staff_ids (assigned).';
