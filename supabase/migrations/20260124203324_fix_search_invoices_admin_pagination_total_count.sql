-- Migration: Fix pagination total count bug in search_invoices_admin
-- Description: The function was counting rows from the paginated result set instead of
--              counting all filtered invoices before pagination. This caused pagination
--              to show incorrect total counts (e.g., "1 of 1" when there are more invoices).
-- Fix: Add a total_count CTE that counts from filtered_invoices BEFORE pagination,
--      then use that count instead of counting the paginated results.
...
BEGIN
  ...
  -- Get total count from filtered_invoices BEFORE pagination
  total_count AS (
...
-- Add comment
COMMENT ON FUNCTION search_invoices_admin IS 'Admin search function for invoices with filtering, pagination, and ordering support. Fixed pagination total count bug.';
