...
CREATE POLICY "Allow admin insert" ON students FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM staff WHERE user_id = auth.uid() AND role = 'ADMIN')
);
CREATE POLICY "Allow admin update" ON students FOR UPDATE USING (
  EXISTS (SELECT 1 FROM staff WHERE user_id = auth.uid() AND role = 'ADMIN')
);
CREATE POLICY "Allow admin delete" ON students FOR DELETE USING (
  EXISTS (SELECT 1 FROM staff WHERE user_id = auth.uid() AND role = 'ADMIN')
);
