-- Include short_name and long_name in search_sessions_admin session rows (used by tutor log session picker, etc.).
...
BEGIN
  ...
  WITH filtered_sessions AS (
    SELECT s.id, s.type, s.class_id, s.subject_id, s.admin_shift_id, s.start_at, s.end_at, s.status, s.created_at, s.updated_at, s.short_name, s.long_name
    FROM public.sessions s
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS
  'Search sessions. Session rows include short_name and long_name. Class sessions: extra = not enrolled. Classless meetings: all students extra for removal UX. Search includes linked parent names.';
