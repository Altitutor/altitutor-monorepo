-- Migration: Fix search_sessions_admin enum comparison and recreate tutor views
-- Description:
--   - Fix search_sessions_admin function to cast enum to text for comparison (fixes schema lint error)
--   - Recreate vtutor_topics and vtutor_topics_files views that were dropped when vtutor_subjects was recreated

-- ========================
-- FIX SEARCH_SESSIONS_ADMIN FUNCTION
-- ========================

-- Fix the enum comparison issue by casting session_type enum to text
-- This fixes the schema lint error: "operator does not exist: session_type = text"
-- We use CREATE OR REPLACE to update the existing function with just the enum cast fix
...
BEGIN
  ...
  WITH filtered_sessions AS (
    ...
    WHERE 
      ...
      -- Type filter (cast enum to text for comparison) - THIS IS THE FIX
...
-- ========================
-- RECREATE VTUTOR_TOPICS_FILES VIEW
-- ========================
