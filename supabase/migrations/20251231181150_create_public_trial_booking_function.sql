-- Migration: Create public trial booking function
-- Description:
--   - Create function to handle public trial session bookings
--   - Check for existing students (block if found)
--   - Link to existing parents if email matches
--   - Create new student with TRIAL status
--   - Create trial session booking
...
COMMENT ON FUNCTION public.create_public_trial_booking IS 'Create a public trial session booking. Blocks if student email already exists. Links to existing parent if email matches.';

-- Grant execute permission to anonymous and authenticated users
GRANT EXECUTE ON FUNCTION public.create_public_trial_booking TO anon, authenticated;

-- Ensure get_available_slots is accessible to anonymous users
