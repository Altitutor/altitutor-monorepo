-- Migration: Extend search_sessions_admin attendance metadata payload
-- Description:
--   - Add planned attendance metadata for students (credited/rescheduled/trial/reschedule target)
--   - Add planned attendance metadata for staff (swapped/swapped target)
--   - Add richer actual attendance metadata (student was_trial, staff type)
...
BEGIN
  ...
  IF p_include_relationships THEN
    ...
    -- Build actual attendance maps (session_id -> student_id/staff_id -> attendance metadata)
    ...
    IF v_tutor_log_ids IS NOT NULL AND array_length(v_tutor_log_ids, 1) > 0 THEN
      ...
      WITH student_attendance_by_session AS (
        SELECT 
          tl.session_id,
          jsonb_object_agg(
            tlsa.student_id::TEXT,
            jsonb_build_object(
              'attended', tlsa.attended,
              'was_trial', tlsa.was_trial
            )
          ) as attendance_map
    ...
    session_students_with_invoice AS (
      SELECT 
        ...
        COALESCE(ss.was_trial, false) as was_trial,
        COALESCE(ss.is_rescheduled, false) as is_rescheduled,
        COALESCE(ss.is_credited, false) as is_credited,
        CASE
          ...
          ELSE jsonb_build_object(
            'session', jsonb_build_object(
              ...
              'class', CASE
                ...
                ELSE jsonb_build_object('start_time', rc.start_time)
              END
            )
          )
        END as rescheduled_session,
        COALESCE(
          (v_actual_student_attendance->ss.session_id::TEXT->st.id::TEXT->>'attended')::boolean,
          NULL
        ) as actual_attended,
        COALESCE(
          (v_actual_student_attendance->ss.session_id::TEXT->st.id::TEXT->>'was_trial')::boolean,
          NULL
        ) as actual_was_trial,
      ...
      SELECT 
        ...
        jsonb_agg(
          jsonb_build_object(
            ...
            'rescheduled_session', rescheduled_session,
            'actual_attended', actual_attended,
            'actual_was_trial', actual_was_trial,
      ...
      SELECT 
        ...
        jsonb_agg(
          jsonb_build_object(
            ...
            'swapped_staff', CASE
              WHEN swapped_stf.id IS NULL THEN NULL
              ELSE jsonb_build_object(
                'id', swapped_stf.id,
                'first_name', swapped_stf.first_name,
                'last_name', swapped_stf.last_name
              )
            END,
            'actual_attended', COALESCE(
              (v_actual_staff_attendance->sf.session_id::TEXT->stf.id::TEXT->>'attended')::boolean,
              NULL
            ),
            'actual_type', COALESCE(
              (v_actual_staff_attendance->sf.session_id::TEXT->stf.id::TEXT->>'type'),
              NULL
            ),
