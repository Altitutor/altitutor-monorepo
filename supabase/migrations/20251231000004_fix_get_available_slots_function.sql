-- Migration: Fix get_available_slots function
-- Description: The function in the database has bugs:
--   1. Checking sess.staff_id directly instead of using sessions_staff junction table
--   2. Wrong weekend availability logic (not checking time of day)
--   3. Missing cleanup of expired reservations at start
-- This migration fixes the function to match the correct implementation
    ...
    WHILE v_slot_start_time < v_opening_end LOOP
      ...
      FROM (
        ...
        WHERE s.status = 'ACTIVE'
          ...
          -- Exclude existing sessions (FIXED: use sessions_staff junction table)
          AND NOT EXISTS (
...
COMMENT ON FUNCTION public.get_available_slots IS 'Calculate available booking slots based on opening hours, staff availability, blockouts, existing sessions, and reservations. Fixed to use sessions_staff junction table and correct weekend time-based availability logic.';
