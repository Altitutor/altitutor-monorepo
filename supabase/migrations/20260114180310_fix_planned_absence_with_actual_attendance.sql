-- Migration: Fix Reconciliation View and Billing for Planned Absences with Actual Attendance
-- Description:
--  - Update vadmin_reconciliation_uninvoiced_sessions to include planned absences that actually attended
--  - View should show sessions where planned_absence = true but tutor log shows attended = true
-- Purpose: Enable reconciliation and invoicing of students who were marked absent but actually attended
...
COMMENT ON VIEW public.vadmin_reconciliation_uninvoiced_sessions IS 
  'Admin view: Past sessions without invoice items (need manual invoicing). Includes planned absences that actually attended (tutor log shows attended = true). Includes session name (Adelaide timezone) and attendance fields.';
