-- Migration: Add has_credit_notes to invoices and consistent "Paid (Refunded)" display
-- Description:
--   - Add invoices.has_credit_notes so list/session views can show refunded status when invoice has credit notes.
--   - search_invoices_admin returns is_refunded and has_credit_notes for badge display.
--   - search_sessions_admin exposes invoice_status as 'paid_refunded' when paid and (is_refunded or has_credit_notes).

-- ================================================
-- 1. ADD has_credit_notes TO INVOICES
-- ================================================

ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS has_credit_notes BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN public.invoices.has_credit_notes IS 'True when invoice has at least one non-void credit note (for display as Paid (Refunded))';

-- Backfill from existing credit_notes
UPDATE public.invoices inv
SET has_credit_notes = true
WHERE EXISTS (
  SELECT 1 FROM public.credit_notes cn
  WHERE cn.invoice_id = inv.id AND cn.status IS DISTINCT FROM 'void'
);

-- ================================================
-- 2. UPDATE search_invoices_admin (return is_refunded, has_credit_notes)
-- ================================================
...
COMMENT ON FUNCTION search_invoices_admin IS 'Admin search invoices with filtering, pagination, ordering. Returns is_refunded and has_credit_notes for Paid (Refunded) display.';

-- ================================================
-- 3. UPDATE search_sessions_admin (invoice_status = 'paid_refunded' when paid and refunded or has credit notes)
-- ================================================
-- Only the invoice_status_per_student and session_students_with_invoice CTEs are changed.
-- We replace the whole function to keep a single source of truth (see 20260309200000 for full context).
...
BEGIN
  ...
  IF p_include_relationships THEN
    ...
    WITH invoice_status_per_student AS (
      SELECT DISTINCT ON (ii.sessions_students_id) ii.sessions_students_id, inv.status as invoice_status, inv.is_refunded as invoice_is_refunded, inv.has_credit_notes as invoice_has_credit_notes
    ...
    session_students_with_invoice AS (
      SELECT ss.session_id, ss.id as sessions_students_id, st.id as student_id, st.first_name, st.last_name, st.status,
        ...
        CASE WHEN iss.invoice_status = 'paid' AND (COALESCE(iss.invoice_is_refunded, false) OR COALESCE(iss.invoice_has_credit_notes, false)) THEN 'paid_refunded' WHEN iss.invoice_status IS NOT NULL THEN iss.invoice_status ELSE NULL END as invoice_status,
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS 'Search and filter sessions with relationships. Uses sessions.short_name/long_name for search. Returns sessionStaff with sessions_staff_id, was_trial, and actual_was_trial. Invoice status is paid_refunded when paid and (is_refunded or has_credit_notes).';
