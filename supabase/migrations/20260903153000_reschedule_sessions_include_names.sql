-- Include stored session/class/subject names in get_available_reschedule_sessions.
-- The function was created before sessions.short_name/long_name and
-- classes.short_name/long_name existed, so replacement pickers had no title.
  ...
  LOOP
    ...
    v_session_array := v_session_array || jsonb_build_array(
      jsonb_build_object(
        ...
        'room', v_session.room,
        'short_name', v_session.short_name,
        'long_name', v_session.long_name,
...
GRANT EXECUTE ON FUNCTION public.get_available_reschedule_sessions(UUID, UUID, INTEGER) TO authenticated;

COMMENT ON FUNCTION public.get_available_reschedule_sessions(UUID, UUID, INTEGER) IS
  'Returns available sessions for rescheduling: same subject, different class, future sessions, student not enrolled, within date range. Includes stored session/class/subject short_name and long_name.';
