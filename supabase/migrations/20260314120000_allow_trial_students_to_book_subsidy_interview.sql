-- Migration: Allow trial students to book SUBSIDY_INTERVIEW sessions
-- Description: Trial students can currently only book TRIAL_SESSION. This allows them to also
--   book SUBSIDY_INTERVIEW so admins can schedule subsidy interviews before converting to active.
--   TRIAL still cannot book DRAFTING or other session types.
...
BEGIN
  ...
  -- TRIAL students can only book TRIAL_SESSION or SUBSIDY_INTERVIEW (subsidy interview is part of onboarding)
  IF v_student_status = 'TRIAL' AND p_session_type NOT IN ('TRIAL_SESSION', 'SUBSIDY_INTERVIEW') THEN
