-- Migration: Fix get_available_slots timezone bug
-- Description: 
--   The function was checking if slot end time exceeds opening hours by casting
--   TIMESTAMPTZ directly to TIME, which uses UTC timezone instead of Adelaide timezone.
--   This caused all slots to be incorrectly skipped.
...
COMMENT ON FUNCTION public.get_available_slots IS 'Calculate available booking slots based on opening hours, staff availability, blockouts, existing sessions, and reservations. Fixed timezone bug: slot end time check now converts to Adelaide timezone before comparing with opening hours.';
