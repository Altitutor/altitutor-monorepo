-- Migration: Fix create_booking_session function - remove created_by from sessions INSERT
-- Description: The sessions table doesn't have a created_by column (only sessions_staff and sessions_students do).
--              Also change status from 'SCHEDULED' to 'ACTIVE' (valid status values).
--              Add created_by to sessions_students INSERT for auditability.
...
COMMENT ON FUNCTION public.create_booking_session IS 'Create a booking session with automatic staff assignment and student/staff linking. Fixed to remove created_by from sessions INSERT and use ACTIVE status.';
