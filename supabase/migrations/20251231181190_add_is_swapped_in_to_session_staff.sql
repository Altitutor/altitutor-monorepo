-- Migration: Add is_swapped_in flag to session staff in search_sessions_admin
-- Description:
--   - Add is_swapped_in flag to indicate when a staff member has swapped into a session
--   - A staff member is swapped-in if their sessions_staff.id is referenced by another sessions_staff.swapped_sessions_staff_id
...
COMMENT ON FUNCTION search_sessions_admin IS 'Admin search function for sessions with filtering, search, pagination, and relationship loading. Supports exact + fuzzy matching for student names, staff names, and session shortname (class shortname). Includes is_swapped_in flag for staff members who have swapped into sessions.';
