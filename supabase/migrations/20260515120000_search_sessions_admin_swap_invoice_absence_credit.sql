-- Migration: search_sessions_admin — fix swapped_staff join (replacement tutor name),
--   add invoice_id to invoice_status_payload, expose absence_credited_at on student rows.
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS
  'Search sessions. Swapped staff pill uses replacement row staff. invoice_status_payload includes invoice_id. Students include absence_credited_at (sessions_students.credited_at).';
