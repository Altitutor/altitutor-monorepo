-- Migration: Add root-only filter to search_topics_admin
-- Description: Adds p_root_only BOOLEAN flag. When TRUE, restricts results AND
-- total count to root topics (parent_id IS NULL). Fixes the /topics admin page
-- where pagination total included child topics but the UI only rendered roots,
-- causing many root topics to be hidden behind misleading pagination.

DROP FUNCTION IF EXISTS public.search_topics_admin(TEXT, UUID[], INTEGER, INTEGER);

CREATE OR REPLACE FUNCTION public.search_topics_admin(
  -- Search
...
BEGIN
  ...
  WITH filtered_topics AS (
    ...
    WHERE
      ...
      AND (p_root_only = FALSE OR t.parent_id IS NULL)
  ),
  total_count_cte AS (
...
GRANT EXECUTE ON FUNCTION public.search_topics_admin TO authenticated;

COMMENT ON FUNCTION public.search_topics_admin IS 'Admin search function for topics with relevance ranking and subject filtering. p_root_only restricts both results and total count to root topics (parent_id IS NULL), used by hierarchical browse views to avoid mixing children into paginated lists.';
