-- Migration: Exclude Trial Students from Reconciliation View
-- Description:
--  - Update vadmin_reconciliation_uninvoiced_sessions to exclude sessions for students who were TRIAL at the time
--  - Exclude if student is currently TRIAL
--  - Exclude if student is currently ACTIVE but status changed from TRIAL to ACTIVE after session end time
-- Purpose: Prevent billing and reconciliation of trial students' sessions
...
WHERE 
  ...
  -- Exclude trial students: Case 1 - currently TRIAL
  AND st.status != 'TRIAL'
  -- Exclude trial students: Case 2 - currently ACTIVE but was TRIAL during session
  -- (status changed from TRIAL to ACTIVE after session end time)
  AND NOT (
    st.status = 'ACTIVE'
    AND EXISTS (
      SELECT 1
...
COMMENT ON VIEW public.vadmin_reconciliation_uninvoiced_sessions IS 
  'Admin view: Past sessions without invoice items (need manual invoicing). Includes planned absences that actually attended (tutor log shows attended = true). Excludes sessions for students who were TRIAL at the time (currently TRIAL or status changed from TRIAL to ACTIVE after session end). Includes session name (Adelaide timezone) and attendance fields.';
