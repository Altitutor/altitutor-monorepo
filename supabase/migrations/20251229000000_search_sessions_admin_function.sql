-- Migration: Create search_sessions_admin RPC function
-- Description:
--   - Create optimized RPC function for searching and filtering sessions
--   - Supports search by student name, staff name, class shortname/longname
--   - Supports filtering by date range, staff, class, student, status, type
--   - Returns sessions with relationships (students, staff, classes, subjects, tutor logs)
--   - Includes pagination and ordering support
--   - Adds performance indexes for date range queries
...
-- Composite index for class + date filtering
CREATE INDEX IF NOT EXISTS idx_sessions_class_start_at 
  ON public.sessions(class_id, start_at)
  WHERE class_id IS NOT NULL;

-- Index for status filtering
CREATE INDEX IF NOT EXISTS idx_sessions_status_start_at 
  ON public.sessions(status, start_at);

-- Index for type filtering
CREATE INDEX IF NOT EXISTS idx_sessions_type_start_at 
  ON public.sessions(type, start_at);

-- ========================
-- SEARCH_SESSIONS_ADMIN FUNCTION
-- ========================
...
DECLARE
  ...
  v_student_ids UUID[];
  v_staff_ids UUID[];
  v_unplanned_student_ids UUID[];
  v_unplanned_staff_ids UUID[];
...
BEGIN
  ...
  WITH filtered_sessions AS (
    ...
    WHERE 
      ...
      -- Type filter
      AND (p_types IS NULL OR array_length(p_types, 1) IS NULL OR s.type = ANY(p_types))
  ),
  ordered_sessions AS (
    SELECT *
  ...
  IF p_include_relationships THEN
      ...
      SELECT 
        ...
        jsonb_agg(
          jsonb_build_object(
            ...
            'sessions_students_id', ss.id,
            'invoice_status', inv.status
          )
        ) as students
      FROM public.sessions_students ss
      JOIN public.students st ON st.id = ss.student_id
      LEFT JOIN public.invoice_items ii ON ii.sessions_students_id = ss.id
    ...
    -- Fetch unplanned students from tutor logs
    IF v_tutor_log_ids IS NOT NULL AND array_length(v_tutor_log_ids, 1) > 0 THEN
...
-- Note: Function already has SET search_path = public in CREATE statement above

-- ========================
-- COMMENTS
-- ========================
COMMENT ON FUNCTION search_sessions_admin IS 'Admin search function for sessions with filtering, search, pagination, and relationship loading';
COMMENT ON INDEX idx_sessions_start_at IS 'Index for date range filtering on sessions';
COMMENT ON INDEX idx_sessions_class_start_at IS 'Composite index for class + date filtering';
COMMENT ON INDEX idx_sessions_status_start_at IS 'Composite index for status + date filtering';
COMMENT ON INDEX idx_sessions_type_start_at IS 'Composite index for type + date filtering';
