-- Migration: Add was_trial to search_sessions_admin staff payload
-- Description: Include was_trial and actual_was_trial for staff in search_sessions_admin RPC function
--   to support staff "Attending (TRIAL)" status display
--
...
--   " | grep -q "was_trial.*staff\|actual_was_trial" && echo "OK: was_trial present"
...
BEGIN
  ...
  IF p_include_relationships THEN
    ...
    -- Planned session staff: add sessions_staff_id (sf.id) and was_trial for undo-absence in staff tab
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS 'Search and filter sessions with relationships. Returns sessionStaff with sessions_staff_id, was_trial, and actual_was_trial for staff trial status support.';
