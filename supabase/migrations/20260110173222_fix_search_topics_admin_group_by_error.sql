-- Migration: Fix search_topics_admin GROUP BY error
-- Description:
--   - Fix SQL error: column "tc.count" must appear in GROUP BY clause or be used in an aggregate function
--   - When using jsonb_agg() (aggregate function), tc.count must be wrapped in an aggregate
--   - Since total_count always returns exactly one row, use MAX(tc.count) to satisfy SQL requirements
...
COMMENT ON FUNCTION search_topics_admin IS 'Admin search function for topics with exact + fuzzy matching on shortname and longname, and filtering by subject_ids. Returns subjects with short_name and long_name fields. Fixed pagination total count bug and GROUP BY error.';
