-- Migration: Fix create_tutor_log RPC to handle p_notes JSONB parameter correctly
-- Issue: Supabase RPC client may send JavaScript string arrays in a format
-- that PostgreSQL can't directly parse as JSONB, causing "invalid input syntax for type json" error
-- Solution: Explicitly cast p_notes to JSONB to handle any input format
