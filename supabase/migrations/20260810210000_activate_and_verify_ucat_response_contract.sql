...
-- Historical attempts remain valid evidence after their authored question or
-- answer options are soft-deleted. New attempts must still reference active
-- content, but an update that preserves the existing question_id (including
-- this migration's snapshot backfill) validates against the retained rows.
...
REVOKE ALL ON FUNCTION public.validate_ucat_question_attempt_response()
  FROM PUBLIC, anon, authenticated;
