-- Migration: Fix pagination total count bug in search_subjects_admin
-- Description: The function was counting rows from the paginated result set instead of
--              counting all filtered subjects before pagination. This caused pagination
--              to show incorrect total counts (e.g., "1 of 1 . 50 total" when there are
--              more than 50 subjects).
-- Fix: Add a total_count CTE that counts from filtered_subjects BEFORE pagination,
