-- Migration: Add precreate_sessions function and unique index for sessions

-- ========================
-- ADD NEW DATETIME COLUMNS AND UNIQUE INDEX (TIMESTAMPTZ)
-- ========================
...
DROP INDEX IF EXISTS uq_sessions_class_date_time;

-- New unique index using start/end
CREATE UNIQUE INDEX IF NOT EXISTS uq_sessions_class_start_end
ON public.sessions(class_id, start_at, end_at)
WHERE class_id IS NOT NULL;

-- Drop legacy columns if present (no backfill required)
ALTER TABLE public.sessions
...
-- ========================
-- FUNCTION: public.precreate_sessions
-- Precreate missing CLASS sessions between dates, optionally for a single class
-- ========================
