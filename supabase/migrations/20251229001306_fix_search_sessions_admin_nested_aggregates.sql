-- Migration: Fix nested aggregate errors in search_sessions_admin function
-- Description:
--   - Fix nested aggregate function calls in unplanned students/staff sections
--   - Fix variable type declarations (v_unplanned_student_ids and v_unplanned_staff_ids should be JSONB, not UUID[])
--   - This migration replaces the function definition to fix linting errors
...
DECLARE
  ...
  v_student_ids UUID[];
  v_staff_ids UUID[];
  v_unplanned_student_ids JSONB;  -- Fixed: Changed from UUID[] to JSONB
  v_unplanned_staff_ids JSONB;    -- Fixed: Changed from UUID[] to JSONB
