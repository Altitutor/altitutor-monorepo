-- Migration: Update search_sessions_admin to return invoice_status_payload for centralized badge display
-- Replaces invoice_status (string) with invoice_status_payload (object: status, paid_at, refunded_at, refunded_via_cn_at, credited_at)
-- Frontend uses getInvoiceStatusBadge(invoice_status_payload) for consistent multi-pill display everywhere
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS 'Search sessions. Returns invoice_status_payload (status, paid_at, refunded_at, refunded_via_cn_at, credited_at) for centralized badge display.';
