-- Migration: Fix create_public_trial_booking RAISE MESSAGE conflict
-- Description:
--   PostgreSQL rejects RAISE EXCEPTION 'text' USING MESSAGE = '...';
--   (message is already set by the format string). That caused
--   "RAISE optional already specified: MESSAGE" (42601) whenever a
--   public booking reused an existing student email, instead of a clean
--   STUDENT_EXISTS error the student-web API expects.
