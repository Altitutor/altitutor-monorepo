-- Harden unused-private-stem reconciliation to read attachments through
-- vtutor_* façades (works for all UCAT tutors, not only ADMINSTAFF RLS),
-- matching the legacy reconciliation report.
...
BEGIN
  ...
  WITH filtered AS MATERIALIZED (
    ...
    WHERE catalog.deleted_at IS NULL
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM public.vtutor_ucat_sessions_resources session_resource
        WHERE session_resource.question_stem_id = catalog.id
      )
      AND NOT EXISTS (
        SELECT 1
        FROM public.vtutor_ucat_learning_module_blocks block
        WHERE block.question_stem_id = catalog.id
           OR (
             block.question_id IS NOT NULL
             AND EXISTS (
               SELECT 1
               FROM jsonb_array_elements(
                 COALESCE(detail.questions::jsonb, '[]'::jsonb)
               ) AS question
               WHERE NULLIF(question->>'id', '')::uuid = block.question_id
                 AND NULLIF(question->>'deleted_at', '') IS NULL
             )
           )
      )
      AND (
...
COMMENT ON FUNCTION public.tutor_ucat_list_private_stems_not_in_set(
  TEXT, UUID[], INTEGER, INTEGER
) IS
  'Paginated queue of private stems with no set, learning-module, or session attachment.';
