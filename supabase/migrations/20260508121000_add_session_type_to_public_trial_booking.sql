-- Migration: Add session_type parameter to create_public_trial_booking function
-- Description:
--   - Allow public booking flow to create either TRIAL_SESSION or SUBSIDY_INTERVIEW
--   - Keep TRIAL_SESSION as default for backward compatibility

DROP FUNCTION IF EXISTS public.create_public_trial_booking(
  TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, INTEGER, UUID[], TEXT, TEXT, TEXT, TEXT
);
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  -- Only allow public bookings for trial/subsidy interview session types
  IF p_session_type NOT IN ('TRIAL_SESSION', 'SUBSIDY_INTERVIEW') THEN
    RAISE EXCEPTION 'INVALID_SESSION_TYPE'
      USING ERRCODE = 'P0001',
            MESSAGE = 'Session type must be TRIAL_SESSION or SUBSIDY_INTERVIEW';
  END IF;
...
COMMENT ON FUNCTION public.create_public_trial_booking(TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, INTEGER, UUID[], TEXT, TEXT, TEXT, TEXT) IS 'Create a public trial/subsidy booking. Blocks if student email already exists. Links to existing parent if email matches. Assigns subjects if provided.';
