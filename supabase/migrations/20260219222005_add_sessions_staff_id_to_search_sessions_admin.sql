-- Migration: Add sessions_staff_id to search_sessions_admin session staff payload
-- Description: Include sessions_staff.id for each staff row so staff tabs can trigger
--   "Undo log absence" without opening the session modal. Planned staff get sf.id;
--   unplanned staff get NULL (they are not in sessions_staff).
--
-- Verify after applying (Supabase running, e.g. after supabase db reset):
--   psql "postgresql://postgres:postgres@127.0.0.1:55322/postgres" -t -c "
--   SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname = 'search_sessions_admin';
--   " | grep -q sessions_staff_id && echo "OK: sessions_staff_id present"
--   Or with data: call RPC with explicit casts; sessionStaff entries should include sessions_staff_id.
    ...
    FROM (
      ...
      WHERE (
        LOWER(format_class_short_name(
          c.day_of_week, c.start_time::time,
          subj.curriculum::text, subj.year_level, subj.name
        )) LIKE '%' || v_search_lower || '%'
        OR LOWER(format_class_full_name(
          c.day_of_week, c.start_time::time, c.end_time::time,
          subj.curriculum::text, subj.year_level, subj.name
        )) LIKE '%' || v_search_lower || '%'
        OR (v_search_like IS NOT NULL AND (
  ...
  IF p_include_relationships THEN
    ...
    IF v_tutor_log_ids IS NOT NULL AND array_length(v_tutor_log_ids, 1) > 0 THEN
      ...
      WITH staff_attendance_by_session AS (
        SELECT tl.session_id,
          jsonb_object_agg(tlsf.staff_id::TEXT, jsonb_build_object('attended', tlsf.attended, 'type', tlsf.type)) as attendance_map
    ...
    -- Planned session staff: add sessions_staff_id (sf.id) for undo-absence in staff tab
    ...
    IF v_tutor_log_ids IS NOT NULL AND array_length(v_tutor_log_ids, 1) > 0 THEN
      WITH unplanned_staff AS (
        SELECT DISTINCT tl.session_id, tlsf.staff_id, tlsf.attended, tlsf.type
        FROM public.tutor_logs_staff_attendance tlsf
        JOIN public.tutor_logs tl ON tl.id = tlsf.tutor_log_id
        WHERE tl.session_id = ANY(v_session_ids)
      ...
      unplanned_staff_details AS (
        SELECT us.session_id, stf.id, stf.first_name, stf.last_name, stf.role, stf.status, us.attended, us.type
...
COMMENT ON FUNCTION search_sessions_admin(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, UUID, UUID, UUID, UUID, TEXT[], TEXT[], BOOLEAN, INTEGER, INTEGER, TEXT, BOOLEAN) IS 'Search and filter sessions with relationships. Returns sessionStaff with sessions_staff_id for undo absence in staff tab.';
