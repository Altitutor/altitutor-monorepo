...
import { formatSessionDate, getClassLongName } from './utils.ts';
...
export interface ProcessStudentDependencies {
  ...
  subjectById: Record<
    string,
    { name?: string; curriculum?: string; year_level?: number }
  >;
  ...
  billingAdjustment?: {
    id: string;
    kind: 'session_charge' | 'restoration_charge';
...
export async function processStudentInvoicing(
  ...
  try {
    ...
    for (const item of studentSessions) {
      ...
      const priceResult = billingAdjustment?.kind === 'restoration_charge'
        ? {
          amount_cents: billingAdjustment.amountCents ?? 0,
          currency: billingAdjustment.currency ?? 'AUD',
        }
      ...
      invoiceItems.push({
        ...
        line_kind: billingAdjustment?.kind ?? 'session_charge',
        restores_credit_note_id: billingAdjustment?.restoresCreditNoteId,
    ...
    if (invoiceItems.length === 0) {
      console.log(
        `${LOG_PREFIX} No invoice items for student ${studentId} on ${invoiceDate}, skipping`,
      );
      return {
        invoiceId: null,
        error: `No invoice items generated. All sessions may have been skipped due to validation errors.${
    ...
    if (!billing?.stripe_customer_id) {
      try {
        ...
        console.error(
          ...
          formatStripeErrorMessage(createErr, 'create billing account', {
            studentId,
          }),
        );
    ...
    const shouldAutoBill = autoBillEnabled &&
    ...
    const hasPriorInvoiceLines = sessionsStudentsIds.length > 0 &&
      (await hasAnyInvoiceItemForSessions(supabase, sessionsStudentsIds));
    const stripeInvoiceCreateNonce = billingAdjustment?.id ??
      (hasPriorInvoiceLines ? `${timestamp}_${crypto.randomUUID()}` : undefined);
    ...
    if (!shouldAutoBill) {
      ...
      try {
        ...
        let stripeInvoiceItems: Array<
          InvoiceItemRow & { stripe_invoice_item_id: string }
        > = [];
        try {
          ...
          stripeInvoiceItems = result.stripeInvoiceItems as Array<
            InvoiceItemRow & { stripe_invoice_item_id: string }
          >;
        } catch (itemErr: unknown) {
          console.error(
            `${LOG_PREFIX} Failed to create invoice items for student ${studentId}, deleting draft invoice:`,
            formatStripeErrorMessage(itemErr, 'create invoice items', {
              studentId,
            }),
          );
          try {
            ...
            console.error(
              ...
              formatStripeErrorMessage(delErr, 'delete draft invoice', {
                studentId,
              }),
            );
          }
        ...
        try {
          ...
          console.error(
            ...
            formatStripeErrorMessage(finalizeErr, 'finalize invoice', {
              studentId,
            }),
          );
          try {
        ...
        const dbInvoice = await saveInvoiceToDatabase(
          supabase,
          studentId,
          finalizedInvoice,
          invoiceDate,
        ...
        if (!dbInvoice) {
          ...
          try {
            ...
            console.log(
              `${LOG_PREFIX} Successfully voided invoice ${finalizedInvoice.id} during rollback`,
            );
          } catch (voidErr) {
            getStripeErrorDetails(voidErr);
            console.error(
              `${LOG_PREFIX} Failed to void invoice ${finalizedInvoice.id} during rollback:`,
              formatStripeErrorMessage(voidErr, 'void invoice', {
                studentId,
        ...
        try {
          await saveInvoiceItemsToDatabase(
            supabase,
            dbInvoice.id,
            stripeInvoiceItems,
          );
          ...
          console.error(
            ...
            formatStripeErrorMessage(itemsErr, 'save invoice items', {
              studentId,
          ...
          try {
            ...
            console.error(
              ...
              formatStripeErrorMessage(
                rollbackErr,
                'rollback failed invoice persist',
                { studentId },
              ),
            );
          }
        ...
        if (resendApiKey) {
          ...
          try {
            ...
            if (emailResult.sent.length > 0) {
              console.log(
                `${LOG_PREFIX} Sent invoice ${finalizedInvoice.id} emails to: ${emailResult.sent.join(', ')}`,
              );
            }
            if (emailResult.failed.length > 0) {
              console.warn(
                `${LOG_PREFIX} Failed to send invoice ${finalizedInvoice.id} emails to: ${
                  emailResult.failed.join(', ')
                }`,
              );
            }
            ...
            console.error(
              ...
              formatStripeErrorMessage(emailErr, 'send invoice email', {
                studentId,
                invoiceId: finalizedInvoice.id,
              }),
            );
          ...
          console.warn(
            `${LOG_PREFIX} RESEND_API_KEY not configured, skipping invoice email for ${finalizedInvoice.id}`,
          );
        }
        ...
        console.error(
          ...
          formatStripeErrorMessage(e, 'create invoice (send_invoice)', {
            studentId,
          }),
        );
        return {
      ...
      try {
        ...
        console.error(
          `${LOG_PREFIX} Failed to create invoice for student ${studentId}:`,
          formatStripeErrorMessage(e, 'create invoice (charge_automatically)', {
            studentId,
          }),
        );
        return {
    ...
    return {
      invoiceId: null,
      error: formatStripeErrorMessage(e, 'process student invoicing', {
        studentId,
      }),
    };
  }
