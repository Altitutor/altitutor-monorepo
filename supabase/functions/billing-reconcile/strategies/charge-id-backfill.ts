...
export async function reconcileChargeIdBackfill(
  ...
  if (invoicesError) {
    console.error(
      "[charge-id-backfill] Error fetching invoices:",
      invoicesError,
    );
    return {
  ...
  console.log(
    `[charge-id-backfill] Found ${invoices.length} paid invoice(s) missing stripe_charge_id`,
  );

  if (!fixBackfill) {
    for (const invoice of invoices) {
      const warning =
        `Invoice ${invoice.stripe_invoice_id}: Missing charge_id - ` +
        "report-only mode skipped the Stripe lookup";
  ...
  for (let i = 0; i < invoices.length; i++) {
    ...
    try {
      ...
      const raw = stripeInvoice as {
        charge?: string | null;
        payment_intent?: string | null;
      };
      const chargeId = typeof raw.charge === "string" ? raw.charge : null;
      const paymentIntentId = typeof raw.payment_intent === "string"
        ? raw.payment_intent
        : null;

      if (!chargeId && !paymentIntentId) {
        skipped.push(
          `${invoice.stripe_invoice_id}: No charge or payment_intent on Stripe (e.g. customer balance only)`,
        );
        continue;
      }
      ...
      if (chargeId) {
        changes.push({
          ...
          old_value: invoice.stripe_charge_id,
          new_value: chargeId,
        });
      }
      if (paymentIntentId) {
        changes.push({
          ...
          old_value: invoice.stripe_payment_intent_id,
          new_value: paymentIntentId,
        });
      }
      ...
      if (updateErr) {
        errors.push(
          `Invoice ${invoice.stripe_invoice_id}: ${getErrorMessage(updateErr)}`,
        );
        ...
        console.log(
          `[charge-id-backfill] Backfilled charge_id for ${invoice.stripe_invoice_id}`,
        );
      }
    } catch (err: unknown) {
