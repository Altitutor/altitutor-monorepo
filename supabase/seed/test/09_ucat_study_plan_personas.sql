...
VALUES
  ...
  ('f5000000-0000-4000-8000-000000000004', '10000000-0000-0000-0000-000000000004', 2200, EXTRACT(YEAR FROM CURRENT_DATE + 90)::INT, CURRENT_DATE + 90, jsonb_build_array(jsonb_build_object('weekday', EXTRACT(DOW FROM CURRENT_DATE)::INT, 'maxMinutes', 60), jsonb_build_object('weekday', (EXTRACT(DOW FROM CURRENT_DATE)::INT + 2) % 7, 'maxMinutes', 60), jsonb_build_object('weekday', (EXTRACT(DOW FROM CURRENT_DATE)::INT + 4) % 7, 'maxMinutes', 60)), EXTRACT(DOW FROM CURRENT_DATE)::INT, NOW(), NULL, NULL),
