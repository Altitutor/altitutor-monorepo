...
select throws_ok(format('select public.admin_work_item_change(''task'',%L,%L,''stale-after-ui'',''{"status":"done"}'')',(select body#>>'{record,id}' from work where kind='task'),(select body->>'revision' from work where kind='task')),'40001',null,'intervening UI changes invalidate the agent revision');
