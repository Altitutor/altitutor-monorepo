...
describe('buildDashboardDayUpdates', () => {
  ...
  it('lists meetings and omits class and admin-shift sessions', () => {
    const updates = buildDashboardDayUpdates({
      sessions: [
        ...
        session({ id: 'check-in-1', type: 'CHECK_IN', class_id: null, short_name: null }),
        session({ id: 'trial-1', type: 'TRIAL_SESSION', class_id: null, short_name: 'TRIAL sun 6 sep 11:15' }),
        session({ id: 'subsidy-1', type: 'SUBSIDY_INTERVIEW', class_id: null, short_name: null }),
        session({ id: 'shift-1', type: 'ADMIN_SHIFT', class_id: null, short_name: null }),
  ...
  it('does not list extra students on meetings', () => {
    const updates = buildDashboardDayUpdates({
      sessions: [session({ type: 'ADMIN_MEETING', class_id: null, short_name: null })],
      sessionStudents: {
