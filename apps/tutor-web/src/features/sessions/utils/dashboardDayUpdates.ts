...
function formatSessionType(type: string | null | undefined): string {
...
function sessionLabel(session: DashboardDaySession): string {
  const shortName = session.short_name?.trim();
  if (shortName) return shortName;
  const longName = session.long_name?.trim();
