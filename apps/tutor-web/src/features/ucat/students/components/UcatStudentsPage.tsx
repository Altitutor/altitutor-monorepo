...
import { AccountClassBadge, DataTable, DataTableToolbar, TablePagination } from '@altitutor/ui';
...
import { getCognitivePredictedScore, UcatPredictedScoreCell } from './UcatPredictedScoreCell';

export function UcatStudentsPage() {
  const access = useUcatAccess();
  const progress = useUcatStudentProgressSummary();
  const tableState = useUcatTableUrlState(
    ['student_name', 'predicted_score', 'delivery_mode', 'online_tier', 'actions'],
    {
      availableColumns: [
        'student_name',
        'total_questions',
  ...
  const deliveryFilterValue = (tableState.state.filters.delivery_mode?.[0] as string | undefined) ?? 'all';
  const tierFilterValue = (tableState.state.filters.online_tier?.[0] as string | undefined) ?? 'all';

  const rows = useMemo(() => progress.data?.students ?? [], [progress.data?.students]);
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const classHit = classFilterValue === 'all' || row.class_ids.includes(classFilterValue);
      const deliveryHit = deliveryFilterValue === 'all' || row.delivery_mode === deliveryFilterValue;
      const tierHit = tierFilterValue === 'all' || row.online_tier === tierFilterValue;
      return searchHit && classHit && deliveryHit && tierHit;
    });
  }, [rows, tableState.state.search, classFilterValue, deliveryFilterValue, tierFilterValue]);
  ...
  const sortedRows = useMemo(
    () =>
      applySort(filteredRows, tableState.state.sortBy, tableState.state.sortDirection, {
        ...
        predicted_score: (r) => getCognitivePredictedScore(r, progress.data?.sections ?? []) ?? -1,
        delivery_mode: (r) => r.delivery_mode,
        online_tier: (r) => r.online_tier,
        last_attempted_at: (r) => r.last_attempted_at ?? '',
      }),
    [filteredRows, tableState.state.sortBy, tableState.state.sortDirection, progress.data?.sections]
  );
  ...
  const allColumns: Array<{
      ...
      column: {
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <AccountClassBadge accountClass={row.original.account_class} />
          </div>
        ),
      },
    },
    ...
    },
    {
      ...
      column: {
        ...
        accessorFn: (row: StudentProgressSummaryRow) => getCognitivePredictedScore(row, progress.data?.sections ?? []),
        cell: ({ row }: { row: { original: StudentProgressSummaryRow } }) => (
          <UcatPredictedScoreCell student={row.original} sections={progress.data?.sections ?? []} />
        ),
      },
    },
    {
      ...
      column: {
        ...
        header: 'Delivery',
        cell: ({ row }) => (row.original.delivery_mode === 'in_person' ? 'In person' : 'Online'),
      },
    },
    {
      key: 'online_tier',
      column: {
        ...
        header: 'Online plan',
        cell: ({ row }) => (row.original.online_tier === 'unlimited' ? 'Unlimited' : 'Free'),
      },
    },
    {
      ...
      column: {
        ...
        cell: ({ row }) =>
          row.original.last_attempted_at ? new Date(row.original.last_attempted_at).toLocaleString() : '-',
      },
    },
    {
      key: 'actions',
  ...
  const visibleColumns = useVisibleColumns(
    allColumns,
    [...tableState.state.visibleColumns, 'actions'].filter((k, i, arr) => arr.indexOf(k) === i)
  );
  ...
  if (access.isLoading || progress.isLoading) return <UcatPageSkeleton rows={8} />;
    ...
    <div className="space-y-6 py-8 md:py-10">
      ...
      <DataTableToolbar
        ...
        onVisibleColumnsChange={(cols) =>
          tableState.actions.onVisibleColumnsChange([...cols.filter((c) => c !== 'actions'), 'actions'])
        }
