...
export const ucatStudentsApi = {
  ...
  async ucatClasses() {
    const supabase = getSupabaseClient() as SupabaseClient<Database>;
    const { data, error } = await supabase.from('vtutor_classes').select('id,subject_name').eq('subject_name', 'UCAT');
    if (error) throw error;
    return data ?? [];
  },
  ...
  async listUcatClassesWithDetails(): Promise<UcatClassWithDetails[]> {
    ...
    const detailsByClassId = (detailsData ?? []).reduce(
      ...
      {} as Record<
        string,
        {
          students: UcatClassWithDetails['students'];
          staff: UcatClassWithDetails['staff'];
        }
      >
    );
