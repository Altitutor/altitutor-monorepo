...
export async function GET(
  ...
  if (subscriptionError) {
    console.error(
      "Failed to read tutor calendar subscription:",
      subscriptionError,
    );
  ...
  const { data, error } = await admin
    ...
    .eq("staff_id", subscription.staff_id)
    .eq("session.status", "ACTIVE")
    .order("start_at", { referencedTable: "sessions", ascending: true });

  if (error) {
    console.error("Failed to load tutor calendar sessions:", error);
    return new NextResponse("Could not load calendar", { status: 500 });
  }
  ...
  const feed = buildTutorCalendarFeed(sessions, getTutorBaseUrl(request));
