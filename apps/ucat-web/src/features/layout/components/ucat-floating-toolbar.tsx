...
export function UcatFloatingToolbar() {
  ...
  const compact =
    isMobile || tutorial || preferences.examToolbarLayout === "compact_top";
  const toolbarVisible = tutorial || preferences.examToolbarVisible;
  const topToolbarVisible = compact && toolbarVisible;
  const rightToolbarVisible = !compact && toolbarVisible;
  const toggleLabel = tutorial
    ...
    >
      <div className="pointer-events-auto">
        <button
          ...
          className="inline-flex h-9 items-center gap-1.5 rounded-full border bg-background/95 px-3 text-sm shadow-md backdrop-blur transition-[box-shadow,transform,opacity] duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] hover:shadow-lg active:scale-95 disabled:cursor-not-allowed disabled:opacity-60"
        >
