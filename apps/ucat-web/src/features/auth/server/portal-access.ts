...
function captureUnavailable(stage: 'authentication' | 'portal_access', startedAt: number, error: unknown) {
...
function parsePayload(value: unknown): UcatPortalAccessPayload | null {
  ...
  return {
    ...
    signupCompleted: studentId ? Boolean(record.ucat_signup_completed_at) : null,
    signupStep: nullableNumber(record, 'ucat_signup_step') ?? 1,
    unlimitedTrialEligible: boolean(record, 'unlimited_trial_eligible'),
    analyticsAccountClass: record.account_class === 'internal_test' ? 'internal_test' : 'external',
...
async function queryWithClockSkewRetry(operation: () => PromiseLike<QueryResult>, startedAt: number) {
...
export async function resolveUcatPortalAccess(verifiedUserId?: string | null): Promise<UcatPortalAccess> {
  ...
  try {
    ...
    try {
      ...
      captureUnavailable(verifiedUserId === undefined ? 'authentication' : 'portal_access', startedAt, error);
    ...
    if (userId === undefined) {
      ...
      if (claims.error?.name === 'AuthSessionMissingError') return { status: 'unauthenticated' };
      if (claims.error) {
    ...
    try {
      result = await deadline.race(
        queryWithClockSkewRetry(() => supabase.rpc('current_ucat_portal_access'), startedAt)
      );
    } catch (error) {
