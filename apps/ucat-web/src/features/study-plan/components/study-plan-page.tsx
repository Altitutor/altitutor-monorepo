          ...
          <Card className={UCAT_CARD_CHROME}>
            ...
            <CardContent>
              {settingsOpen || !query.data.profile ? (
                ...
                <div className="grid gap-3 text-sm sm:grid-cols-3"><div><span className="text-muted-foreground">Target</span><p className="mt-1 text-lg font-semibold">{query.data.profile.targetScore}</p></div><div><span className="text-muted-foreground">Planning date</span><div className="mt-1 font-medium">{new Date(`${query.data.profile.planningDate}T00:00:00`).toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" })}{query.data.profile.planningDateIsProvisional ? <Badge className="ml-2" variant="secondary">Provisional</Badge> : null}</div></div><div><span className="text-muted-foreground">Study days</span><p className="mt-1 font-medium">{query.data.profile.availableDays.map((item) => DAYS.find((day) => day.value === item.weekday)?.short).join(", ")}</p></div></div>
              )}
            </CardContent>
          </Card>
