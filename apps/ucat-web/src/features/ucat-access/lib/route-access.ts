...
const ONLINE_PREFIXES = ['/learn', '/practice', '/sets', '/mocks']
...
export function getUpsellConfigForPath(path: string): UpsellConfig | null {
  ...
  if (ONLINE_PREFIXES.some((prefix) => normalized === prefix || normalized.startsWith(`${prefix}/`))) {
    return { requiredAccess: 'online', badgeLabel: 'Online' }
  }

  if (IN_PERSON_PREFIXES.some((prefix) => normalized === prefix || normalized.startsWith(`${prefix}/`))) {
...
export function hasAccessForPath(path: string, access: UcatAccessFlags): boolean {
