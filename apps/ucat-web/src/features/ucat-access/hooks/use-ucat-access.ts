...
function parseAnalyticsAccountClass(value: string | null): UcatAccessFlags['analyticsAccountClass'] {
...
function mapAccessRow(data: VstudentUcatMyAccessRow): Omit<UcatAccessFlags, 'isLoading' | 'accessLoadFailed'> {
  return {
    ...
    analyticsAccountClass: parseAnalyticsAccountClass(data.account_class),
    testYear: data.ucat_test_year,
    testDate: data.ucat_test_date,
  };
}

async function fetchUcatAccess(): Promise<Omit<UcatAccessFlags, 'isLoading' | 'accessLoadFailed'>> {
...
export function useUcatAccess(): UcatAccessFlags {
  ...
  }, [user, authLoading, query.isLoading, query.isPending, query.isError, query.data]);
}
