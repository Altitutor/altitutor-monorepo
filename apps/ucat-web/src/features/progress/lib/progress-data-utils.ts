...
import type { ProgressMode, TimeFrameDays, AttemptFilter } from './progress-mode'
...
export function applyAttemptFilterToProgress(
  ...
  const sectionProgress: SectionProgress[] = data.sectionProgress.map((s) => {
    ...
    const averageScaledScore = d.scaledCount > 0 ? d.scaledSum / d.scaledCount : null
    return {
      ...s,
  ...
  for (const [sectionId, cats] of Object.entries(data.sectionCategoryProgress ?? {})) {
...
export function computeSectionProgressFromMockAttempts(
  ...
  const mockSetAttempts = setAttempts.filter(
    (s) => s.studentUcatMockAttemptId && mockIds.has(s.studentUcatMockAttemptId)
  )
...
export function aggregateForGraph<T>(
  ...
  const result = allKeys.map((date) => {
    ...
    const label =
      bucket === 'week' ? formatWeekRangeLabel(date) : undefined
