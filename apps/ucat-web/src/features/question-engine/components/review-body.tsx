    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        <p className="font-normal">
          Review: use this screen to <strong>review</strong> the items and amend your responses.
        </p>
      </div>
      <div className="flex-1 min-h-0 overflow-y-auto border-t border-[#9ba9bd]">
        ...
        >
          <h3 className="text-[12pt] font-normal" style={{ fontFamily: UCAT_FONTS.message }}>
            {sectionTitle}
          </h3>
          <span className="text-[12pt] font-normal" style={{ fontFamily: UCAT_FONTS.message }}>
        ...
        <ul className="divide-y divide-[#9ba9bd] bg-white">
          {rows.map((row) => {
            const statusLabel =
              row.status === 'complete' ? '' : row.status === 'incomplete' ? 'Incomplete' : 'Unseen'
            ...
            return (
              ...
              >
                <button
                  ...
                  title={isFlagged ? 'Flagged for review (click to clear)' : 'Flag for review'}
                >
                  {isFlagged ? (
                    <span
