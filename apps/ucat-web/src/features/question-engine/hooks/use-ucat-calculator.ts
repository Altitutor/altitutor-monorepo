...
type KeyType = 'ON/C' | 'number' | 'operation' | 'immediateoperation' | 'memory'
...
function setDisplayValue(state: CalculatorState, value?: number): CalculatorState {
  let nextNumberValue = value !== undefined ? value : state.numberValue
  if (Number.isFinite(nextNumberValue) && Math.abs(nextNumberValue) <= MAX_ABS_VALUE) {
    nextNumberValue = Math.round(nextNumberValue * 1e8) / 1e8
  }

  if (!Number.isFinite(nextNumberValue) || nextNumberValue > MAX_ABS_VALUE || nextNumberValue < -MAX_ABS_VALUE) {
...
function updateDisplayNumber(state: CalculatorState, numberButton: string): CalculatorState {
  ...
  const canAddDigit =
    displayLength < 8 &&
    ((numberButton === '.' && !textValue.includes('.')) || numberButton !== '.')
  ...
  textValue = (textValue === '0' && numberButton !== '.' ? '' : textValue) + numberButton
  ...
  while (hasOperator(working, ['+', '-', '×', '÷'])) {
    ...
    if (hasOperator(working, ['×', '÷'])) {
      ...
      if (divIndex === -1 || (multIndex !== -1 && multIndex < divIndex)) {
        ...
        tmp = (working[multIndex - 1] as number) * (working[multIndex + 1] as number)
      } else {
        operationIndex = divIndex
        tmp = (working[divIndex - 1] as number) / (working[divIndex + 1] as number)
      }
      ...
      if (subIndex === -1 || (addIndex !== -1 && addIndex < subIndex)) {
        ...
        tmp = (working[addIndex - 1] as number) + (working[addIndex + 1] as number)
      } else {
        operationIndex = subIndex
        tmp = (working[subIndex - 1] as number) - (working[subIndex + 1] as number)
      }
    }
    ...
    priorCalculations = [working[operationIndex - 1] as number, working[operationIndex] as Operation]
...
function updateMemory(
  ...
  if (mButton === 'M+') {
    ...
    if (!Number.isFinite(newValue) || newValue > MAX_ABS_VALUE || newValue < -MAX_ABS_VALUE) {
      return withError(state)
    }
...
function computeNextState(prev: CalculatorState, rawKey: string): CalculatorState {
