...
function confirmationResponse(token: string) {
  ...
  const html =
    '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Unsubscribe from optional emails?</title></head><body style="margin:0;background:#f2f0e9;color:#1a1a1a;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif"><main style="min-height:100vh;display:grid;place-items:center;padding:24px"><section style="width:100%;max-width:520px;overflow:hidden;border:1px solid #dce5e8;border-radius:16px;background:#fff"><header style="padding:28px 32px;background:#0a2941;color:#f2f0e9"><strong style="font-size:24px">Altitutor UCAT</strong><div style="margin-top:6px;color:#b9d1d9;font-size:13px">UCAT preparation from Altitutor</div></header><div style="padding:36px 32px"><h1 style="margin:0 0 14px;color:#0a2941;font-size:28px">Unsubscribe from optional emails?</h1><p style="margin:0;color:#52606a;line-height:1.7">This stops teaching, progress, product-news and offer emails. Account, security, billing and access emails are unaffected.</p><form method="post" action="' +
    action +
    '" style="margin:24px 0 0"><button type="submit" style="border:0;border-radius:10px;background:#0a2941;color:#fff;padding:13px 20px;font:inherit;font-weight:700;cursor:pointer">Unsubscribe</button></form><p style="margin:24px 0 0"><a href="/" style="color:#0a2941;font-weight:700">Return to Altitutor UCAT</a></p></div><footer style="padding:20px 32px;background:#eaf1f3;color:#52606a;font-size:12px">A not-for-profit initiative by Altitutor. Need help? <a href="mailto:admin@altitutor.com" style="color:#0a2941">admin@altitutor.com</a></footer></section></main></body></html>';
...
async function unsubscribe(
  token: string,
  source: "email_link" | "list_unsubscribe_one_click",
) {
...
export async function POST(request: NextRequest) {
  ...
  const result = await unsubscribe(
    token,
    browserForm ? "email_link" : "list_unsubscribe_one_click",
  );
  if (!browserForm) {
  ...
  if (!result.ok) {
    return htmlResponse(
      ...
      result.status === 400
        ...
        : "Please contact admin@altitutor.com and we will help.",
      result.status,
    );
  }
  return htmlResponse(
    "You’re unsubscribed",
    "You will no longer receive optional Altitutor UCAT marketing emails. Account, security, billing and access emails are unaffected.",
  );
}
