...
export type ProgressAttemptFilter = ProgressAttemptSource | "all";
...
type HistoryQuery = PromiseLike<HistoryResult> & {
  ...
  lt: (column: string, value: string) => HistoryQuery;
  not: (column: string, operator: "is", value: null) => HistoryQuery;
...
export async function GET(request: Request) {
  ...
  const source = url.searchParams.get("source") as ProgressAttemptFilter | null;
  ...
  if (exactDate != null && !/^\d{4}-\d{2}-\d{2}$/.test(exactDate)) {
    return NextResponse.json(
      { error: "Invalid attempt date" },
      { status: 400 },
    );
  }
  if (dateTo != null && !/^\d{4}-\d{2}-\d{2}$/.test(dateTo)) {
    return NextResponse.json(
      { error: "Invalid attempt end date" },
      { status: 400 },
    );
  }
  ...
  if (completedOnly) query = query.not("completed_at", "is", null);
  if (days != null) {
  ...
  if (exactDate != null) {
    const rangeEnd = dateTo ?? exactDate;
    const nextDate = new Date(`${rangeEnd}T00:00:00.000Z`);
    nextDate.setUTCDate(nextDate.getUTCDate() + 1);
    query = query
      .gte("completed_at", `${exactDate}T00:00:00.000Z`)
  ...
  const attempts = ((result.data ?? []) as unknown as HistoryRow[]).map(
    (row): ProgressAttemptRow => {
      const name =
        extractTextFromRichJson(row.resource_name as JsonLike) || null;
      if (row.source === "set") {
        return {
          source: "set",
          id: row.id,
