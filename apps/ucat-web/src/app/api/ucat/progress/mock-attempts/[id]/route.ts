...
export async function GET(
  ...
  const { data: setAttemptsRaw, error: setAttemptsError } = await supabase
    ...
    .select(
      'id, question_set_id, score_points, total_points, scaled_score'
    )
  ...
  const attemptsBySetAndQuestion = new Map<string, { score: number | null; timeSpentSeconds: number | null; questionType: 'multiple_choice' | 'syllogism' | null }>()
