...
describe("student calendar feed", () => {
  ...
  it("creates stable events with UTC times, SEQUENCE, and a session deep link", () => {
    ...
    expect(unfoldedFeed).toContain(
      "https://student.altitutor.com/classes?session=55fceebe-f6a3-4fa8-af26-7407671e05cc",
    );
    expect(feed.endsWith("\r\n")).toBe(true);
  });

  it("bumps SEQUENCE when a session is rescheduled so clients replace the event", () => {
    const original = buildStudentCalendarFeed(
      [baseSession],
      "https://student.altitutor.com",
    );
    const rescheduled = buildStudentCalendarFeed(
      [
        {
          ...baseSession,
