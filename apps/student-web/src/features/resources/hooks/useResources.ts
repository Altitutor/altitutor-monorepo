...
export function useResourceAccessBySubject() {
  return useQuery({
    queryKey: ['resources', 'access-by-subject'],
    queryFn: resourcesApi.getAccessBySubjectId,
    staleTime: 60_000,
  });
}
...
export function useResourceTopics(subjectId: string | null) {
  return useQuery({
    queryKey: ['resources', 'topics', subjectId],
    queryFn: () => {
...
export function useResourceTopic(subjectId: string | null, topicCode: string | null) {
  return useQuery({
    queryKey: ['resources', 'topic', subjectId, topicCode],
    queryFn: () => {
...
export function useResourceTopicFile(topicId: string | null, fileCode: string | null) {
  return useQuery({
    queryKey: ['resources', 'topic-file', topicId, fileCode],
    queryFn: () => {
...
export function useResourceSignedFileUrl(topicId: string | null, fileCode: string | null) {
  return useQuery({
    queryKey: ['resources', 'topic-file-url', topicId, fileCode],
    queryFn: async () => {
      if (!topicId || !fileCode) throw new Error('Topic ID and file code are required');
