...
export function StudentTodaySessionsCalendarView({
  date,
  sessions,
  isLoading,
  onOpenSession,
  ...
  const todaySessions = sessions.filter(
    (session): session is StudentSessionWithStaff & { session_id: string; start_at: string } =>
  ...
  if (isLoading) {
    return (
      <p className="px-4 py-8 text-center text-sm text-muted-foreground">Loading today’s sessions…</p>
    );
  }
                  ...
                  {(() => {
                    ...
                    groups.forEach((group) => {
                      ...
                      group.forEach((session, sessionIndex) => {
                        ...
                        const isExtra = session.class_id === null;
                        const isNotAttending = session.planned_absence === true;

                        blocks.push(
                          <div
                            key={session.session_id}
                            className={cn('absolute', isNotAttending && 'opacity-50')}
                            style={{
                              top: `${top}px`,
