...
export async function DELETE(
  ...
  try {
    ...
    if (staff.user_id) {
      const { error: authDeleteError } = await supabaseAdmin.auth.admin.deleteUser(staff.user_id);
      if (authDeleteError) {
        console.warn(`Failed to delete auth user ${staff.user_id}: ${authDeleteError.message}`);
