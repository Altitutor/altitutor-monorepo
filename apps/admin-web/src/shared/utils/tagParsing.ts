/**
 * Tag parsing utilities for inline entity mentions in titles/descriptions
 * 
 * Tag format: @[entityType:entityId:displayText]
 * Example: @[student:abc-123:John Doe]
 */

export type TagEntityType = 'student' | 'staff' | 'parent' | 'class' | 'subject' | 'session' | 'topic' | 'file' | 'invoice' | 'task' | 'issue';
 ...
 * Lenient version to handle potential variations
 */
const TAG_PATTERN = /@\[([^:]+):([^:]+):(.+?)\]/g;
