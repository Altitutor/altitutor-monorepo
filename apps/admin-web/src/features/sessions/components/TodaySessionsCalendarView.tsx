                  ...
                  {(() => {
                    ...
                    groups.forEach((group) => {
                      ...
                      group.forEach((s: any, idx: number) => {
                        ...
                        if (elliotStudent) {
                          fetch('http://127.0.0.1:7242/ingest/03d835b2-9f2b-42e2-a795-53809de736bc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'TodaySessionsCalendarView.tsx:196',message:'Elliot Koh in TodaySessionsCalendarView',data:{is_extra:elliotStudent.is_extra,is_extra_type:typeof elliotStudent.is_extra,student:elliotStudent},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
                        }
                        // #endregion
