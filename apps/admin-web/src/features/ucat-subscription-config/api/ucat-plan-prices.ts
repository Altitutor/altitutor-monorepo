...
export type UcatPlanPriceUpdate = Partial<
  Pick<
    UcatPlanPriceRow,
    "base_price_cents" | "stripe_price_id" | "checkout_enabled"
  >
>;
...
export const ucatPlanPricesApi = {
  async list(): Promise<UcatPlanPriceRow[]> {
    const { data, error } = await (
      getSupabaseClient() as SupabaseClient<Database>
    )
  ...
  async syncBasePriceFromStripe(
    id: string,
  ): Promise<{ base_price_cents: number }> {
    const response = await fetch(
      `/api/ucat-plan-prices/${id}/sync-from-stripe`,
      {
        method: "POST",
