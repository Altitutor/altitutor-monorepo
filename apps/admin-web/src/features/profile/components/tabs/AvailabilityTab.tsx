        ...
        <div>
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={key} className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${profile[key as keyof StaffProfile] ? 'bg-green-500' : 'bg-gray-300'}`} />
                <span className={`text-sm ${profile[key as keyof StaffProfile] ? 'text-foreground' : 'text-muted-foreground'}`}>
                  {label}
                </span>
              </div>
            ))}
