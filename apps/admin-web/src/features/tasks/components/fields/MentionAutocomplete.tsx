...
function getEntityDisplayText(result: EntitySearchResult): { title: string; subtitle: string | null } {
  ...
  if (result.type === 'session') {
    ...
    const startDate = sessionData.start_at
      ? new Date(sessionData.start_at).toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
        })
      : '';
    const startTime = sessionData.start_at
      ? new Date(sessionData.start_at).toLocaleTimeString('en-US', {
          hour: 'numeric',
          minute: '2-digit',
        })
      : '';
    return {
      title: `${subjectName} - ${startDate} ${startTime}`.trim(),
      subtitle: classData?.room || null,
    };
  }
