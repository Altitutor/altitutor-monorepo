...
if [ -z "$DEV_PROJECT_REF" ]; then
    ...
    while IFS='=' read -r key value || [ -n "$key" ]; do
        ...
        # Used: TWILIO_*, IMESSAGE_*, STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, RESEND_API_KEY
        ...
        if [[ "$key" =~ ^TWILIO_ ]] || [[ "$key" =~ ^IMESSAGE_ ]] || [[ "$key" == "STRIPE_SECRET_KEY" ]] || [[ "$key" == "STRIPE_WEBHOOK_SECRET" ]] || [[ "$key" == "RESEND_API_KEY" ]]; then
